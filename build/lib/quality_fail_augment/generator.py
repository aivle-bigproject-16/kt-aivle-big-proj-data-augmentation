...
def _resize(image: Image.Image, long_side: int, upscale: bool) -> tuple[Image.Image, Affine]:
    current = max(image.size)
    if current <= long_side and not upscale:
    ...
    return resized, Affine(a=size[0] / image.width, e=size[1] / image.height)
...
def _object_mask_from_label(
    ...
    if outline not in (None, []):
        for ring in point_rings(outline):
            transformed = [
                (
                    transform.a * x + transform.b * y + transform.xoff,
                    transform.d * x + transform.e * y + transform.yoff,
                )
                for x, y in ring
            ]
            if len(transformed) >= 3:
                draw.polygon(transformed, fill=255)
    ...
    difference = np.abs(luminance - background_value)
    threshold = max(float(np.quantile(difference, 0.65)), 8.0)
    estimated = (difference >= threshold).astype(np.uint8) * 255
    estimated_mask = Image.fromarray(estimated, mode="L")
    if estimated_mask.getbbox() is None:
        raise ValueError("object_mask_confidence_too_low")
    return estimated_mask


def _defect_mask_from_label(
...
def _make_one(
    ...
    source_image = open_normalized(_safe_source(raw_root, row["raw_image_path"]), modality)
    source_label = load_json(_safe_source(raw_root, row["raw_json_path"]))
    transform = Affine()
    original_roi: list[float] | None = None
    if modality == "CT":
        roi = extract_ct_roi(source_label, source_image.width, source_image.height)
    ...
    object_mask = _object_mask_from_label(
        source_label, transform, source_image.size, source_image
    )
    defect_mask = _defect_mask_from_label(
        source_label, transform, source_image.size
    )
    ...
    if quality == "fail":
        ...
        for attempt in range(int(config.get("max_augmentation_retries", 8))):
            try:
                case_seed = stable_seed(int(row["item_seed"]), "retry", attempt)
                ...
                source_image = augmented.image
                transform = base_transform.then(augmented.transform)
    ...
    resized, resize_transform = _resize(
        source_image,
        int(config["resize_long_side"]),
        bool(config.get("upscale_small_images", False)),
    )
    transform = transform.then(resize_transform)
