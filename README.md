# quality-fail-augment 1.7

`CT RGB 품질 PASS FAIL 독립 데이터셋 생성 계획서 v1.7`의 실행 코드다.
...
v1.7은 사람 visual QA 게이트와 `fail_visual_qa.csv` 승인 절차를 사용하지 않는다.
...
```powershell
cd "quality_fail_augment_code_v1.7"
py -3.12 -m venv .venv
...
quality-fail-augment audit-raw `
  ...
  --output "E:\quality_fail_raw_audit_v1.7"
```
...
quality-fail-augment plan `
  --raw-root "E:\103.배터리 불량 이미지 데이터" `
  --config ".\config.40k.json" `
  --output "E:\quality_fail_40k_plan_v1.7"
```
...
quality-fail-augment generate `
  ...
  --output "E:\quality_fail_40k_smoke_v1.7" `
...
quality-fail-augment generate `
  ...
  --config ".\config.40k.json" `
  --plan "E:\quality_fail_40k_plan_v1.7\manifests\generation_plan.csv" `
  --output "E:\quality_fail_40k_v1.7"
```
...
```powershell
quality-fail-augment verify --output "E:\quality_fail_40k_v1.7"
```
